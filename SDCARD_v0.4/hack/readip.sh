...
# Required: numbers.wav -> original file [Counting Numbers Normal Voice 0-12] by dersuperanton/freesound.org
#
# Usage readip.sh -> play the current IP after 5'000 mS delay
#		readip.sh 12.34 read "one two (500mS) three four" 

ipadd="$1" # save the argument passed to the script

# NUMBERS_WAV="/mnt/numbers.wav" or /mnt/mmc01... depends if the p2pcam remount differently the sdcard
[ -f /mnt/numbers.wav ] && NUMBERS_WAV="/mnt/numbers.wav" || NUMBERS_WAV="/mnt/mmc01/0/numbers.wav"
...
generate_ip_wav() {
    ...
    while [ $i -le $LEN ]; do
        ...
        case "$C" in
            [0-9])
                ...
                # Direct copy of the number chunk
                dd if="$NUMBERS_WAV" bs=1 skip=$OFFSET count=$NUM_LEN 2>/dev/null >> "$OUTPUT_WAV"
                TOTAL_DATA_LEN=$(( TOTAL_DATA_LEN + NUM_LEN ))

                # Pause 50ms (400 byte A-LAW silence)
                dd if=/dev/zero bs=1 count=$PAUSE_LEN 2>/dev/null \
    ...
    printf $(printf '\\x%02x\\x%02x\\x%02x\\x%02x' \
        ...
        $(((RIFF_SIZE>>16)&0xFF)) \
        $(((RIFF_SIZE>>24)&0xFF))) \
        | dd of="$OUTPUT_WAV" bs=1 seek=4 count=4 conv=notrunc 2>/dev/null
    ...
    printf $(printf '\\x%02x\\x%02x\\x%02x\\x%02x' \
        ...
        $(((DATA_SIZE>>16)&0xFF)) \
        $(((DATA_SIZE>>24)&0xFF))) \
        | dd of="$OUTPUT_WAV" bs=1 seek=40 count=4 conv=notrunc 2>/dev/null
}


##########################################################################################
# If running without parameter, retrive the current IP, generate the wav file and play it.
#
if [ -z $ipadd ]; then
